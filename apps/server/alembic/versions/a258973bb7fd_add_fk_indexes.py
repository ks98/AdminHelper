...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_frp_tunnels_frp_config_id'), 'frp_tunnels', ['frp_config_id'], unique=False)
    op.create_index(op.f('ix_frp_tunnels_connection_id'), 'frp_tunnels', ['connection_id'], unique=False)
    op.create_index(op.f('ix_provision_tokens_server_id'), 'provision_tokens', ['server_id'], unique=False)
